...
class Envs:
    ...
    # Write the SWA KV cache through the bf16-KV intermediate path
    # (_compute_kv_bf16 + backend store_cache) instead of the fused
    # norm+rope+quant kernel, so the fp8 nope payload is quantized from
    # bf16-rounded values -- matching the trainer-side QAT and the DSA-CP
    # path. RL logprob-alignment toggle; costs an extra bf16 KV
    # materialization plus separate store kernels.
